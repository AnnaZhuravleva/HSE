...
def mystem_xml():
    for root, dirs, files in os.walk ('plain'):
        for f in files:
            ...
            mystem_plain = mystem + ' ' + root + os.sep + f + ' ' + goalDir + os.sep + f.replace('.txt', '.xml') + \
                           ' -cid --format xml'
            os.system(mystem_plain)
...
mystem = r'mystem.exe'
columns = ["path", 'author', 'sex', 'birthday', 'header', 'created', 'sphere', 'genre_fi', 'type',
           'topic','chronotop','style','audience_age', 'audience_level', 'audience_size', 'source', 'publication',
...
for page in pages:
    ...
    meta = metadate(html)
    row_csv = [path, meta[0][1], '', '', meta[1][1], meta[2][1], 'публицистика', '', '', meta[3][1], '', 'нейтральный',
               'н-возраст', 'н-уровень','областная', page, 'Урал', '', meta[4], 'газета', 'Россия',
